...
def get_args():
    ...
    parser.add_argument('--add_task_prefix', action='store_true',
                        help="Add 'translate English to SQL: ' prefix to natural language queries (helpful for training from scratch)")
    parser.add_argument('--eval_num_beams', type=int, default=1,
                        help="Number of beams for beam search during validation (1=greedy, 4=beam search). Higher is slower but more accurate.")
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(train_loader):
        ...
        # decoder_targets shape: [batch, target_len] -> [batch*target_len]
        # Note: seq_len should equal target_len after padding in collate function
        logits_flat = logits.view(-1, logits.size(-1))
        targets_flat = decoder_targets.view(-1)
        loss = criterion(logits_flat, targets_flat)
        loss.backward()
        
        # Gradient clipping for training stability (especially important for training from scratch)
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_inputs in tqdm(dev_loader, desc="Evaluating"):
            ...
            # Use configurable beam search (1=greedy for speed, 4=beam search for accuracy)
            ...
            for gen_ids in generated_ids:
                # Remove the decoder start token and decode
    ...
    # Compute error rate
    error_rate = sum(1 for msg in error_msgs if msg != "") / len(error_msgs) if error_msgs else 0.0
...
def main():
    ...
    # Test set
    model_sql_path = os.path.join(f'results/t5_{experiment_name}_test.sql')
    model_record_path = os.path.join(f'records/t5_{experiment_name}_test.pkl')
